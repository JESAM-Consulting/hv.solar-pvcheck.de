...
export default function Step9() {
  ...
  const onSubmit = async (data) => {
    ...
    await axios.post(`https://fe-partnerportal.de/FE_API/lead_api/v1/qualified_lead`, sendData, config).then((res) => {
  ...
  return (
    <div data-show={step == 9 ? true : false} className="step">
      <h1 className="font-visby lg:text-3xl text-xl font-bold lg:max-w-4xl md:max-w-sm">
        Erhalten Sie jetzt Ihr unverbindliches Angebot von Ihrem Fachberater{" "}
        {getData()?.Name}
      </h1>
      <form ref={myForm}
